...
export function GoalsTable({ snapshot, currentRevenueUsd }: Props) {
  ...
  const rows: GoalRow[] = [
    {
      goal: "💵 Gross revenue",
      current: fmtMoney(currentRevenueUsd),
      target: "$120,000",
      pct: (currentRevenueUsd / 120_000) * 100,
    },
    {
          ...
          {rows.map((r) => {
            ...
            return (
              <tr key={r.goal} className="border-t border-zinc-100 dark:border-zinc-800/60">
                ...
                <td className="px-4 py-3 font-medium">{r.current}</td>
